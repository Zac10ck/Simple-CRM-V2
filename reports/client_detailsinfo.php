...
//
// Table class for client_details
//
class cclient_details extends cTable {
	var $date;
	var $client;
	var $person;
	var $desig;
	...
	function __construct() {
		...
		$this->TableVar = 'client_details';
		$this->TableName = 'client_details';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`client_details`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// date
		$this->date = new cField('client_details', 'client_details', 'x_date', 'date', '`date`', ew_CastDateFieldForLike('`date`', 0, "DB"), 133, 0, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// client
		$this->client = new cField('client_details', 'client_details', 'x_client', 'client', '`client`', '`client`', 200, -1, FALSE, '`client`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->client->Sortable = TRUE; // Allow sort
		$this->fields['client'] = &$this->client;

		// person
		$this->person = new cField('client_details', 'client_details', 'x_person', 'person', '`person`', '`person`', 200, -1, FALSE, '`person`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->person->Sortable = TRUE; // Allow sort
		$this->fields['person'] = &$this->person;

		// desig
		$this->desig = new cField('client_details', 'client_details', 'x_desig', 'desig', '`desig`', '`desig`', 200, -1, FALSE, '`desig`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->desig->Sortable = TRUE; // Allow sort
		$this->fields['desig'] = &$this->desig;

		// phone
		$this->phone = new cField('client_details', 'client_details', 'x_phone', 'phone', '`phone`', '`phone`', 131, -1, FALSE, '`phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->phone->Sortable = TRUE; // Allow sort
		$this->phone->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['phone'] = &$this->phone;

		// remark
		$this->remark = new cField('client_details', 'client_details', 'x_remark', 'remark', '`remark`', '`remark`', 200, -1, FALSE, '`remark`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->remark->Sortable = TRUE; // Allow sort
		$this->fields['remark'] = &$this->remark;

		// followup
		$this->followup = new cField('client_details', 'client_details', 'x_followup', 'followup', '`followup`', '`followup`', 200, -1, FALSE, '`followup`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`client_details`";
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "client_detailsview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "client_detailsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "client_detailsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("client_detailsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("client_detailsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "client_detailsadd.php?" . $this->UrlParm($parm);
		else
			$url = "client_detailsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("client_detailsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("client_detailsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("client_detailsdelete.php", $this->UrlParm());
	}
